...
@endsection
@section('titleaction')
<a href="/settings/addgroup"><button class="btn btn-raised btn-primary btn-default btn-sm" style="margin-left: 10px;" type="button">Add New</button></a>
@endsection
				...
				<table class="table">
					...
					<tbody>
						...
						<tr>
							...
							<td>{{ $g['desc'] }}</td>
							<td>
								<a href="/settings/editgroup/{{ $g['id'] }}" style="float: left">
								...
								<form action="/settings/groups/{{ $g['id'] }}" method="POST" style="float: left">
									{{ csrf_field() }}
									{{ method_field('DELETE') }}
