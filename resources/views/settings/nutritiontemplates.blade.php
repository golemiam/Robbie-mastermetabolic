...
@endsection
@section('titleaction')
<a href="/settings/addnutritiontemplate"><button class="btn btn-raised btn-primary btn-default btn-sm" style="margin-left: 10px;" type="button">Add New</button></a>
@endsection
				...
				<table class="table">
					...
					<tbody>
						...
						<tr>
							<td>{{ $template['name'] }}</td>
							<td>
								<a href="/settings/editnutritiontemplate/{{ $template['id'] }}" style="float: left">
								...
								<form action="/settings/nutritiontemplates/{{ $template['id'] }}" method="POST" style="float: left">
									{{ csrf_field() }}
									{{ method_field('DELETE') }}
