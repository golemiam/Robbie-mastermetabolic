...
@endsection
@section('titleaction')
<a href="/settings/addcircuit"><button class="btn btn-raised btn-primary btn-default btn-sm" style="margin-left: 10px;" type="button">Add New</button></a>
@endsection
				...
				<table class="table">
					...
					<tbody>
						...
						<tr>
							<td>{{ $circuit['name'] }}</td>
							<td>
								<a href="/settings/editcircuit/{{ $circuit['id'] }}" style="float: left">
								...
								<form action="/settings/circuits/{{ $circuit['id'] }}" method="POST" style="float: left">
									{{ csrf_field() }}
									{{ method_field('DELETE') }}
