...
@endsection
@section('titleaction')
<a href="/settings/addmeal"><button class="btn btn-raised btn-primary btn-default btn-sm" style="margin-left: 10px;" type="button">Add New</button></a>
@endsection
				...
				<table class="table">
					...
					<tbody>
						...
						<tr>
							<td>{{ $meal['name'] }}</td>
							<td>
								<a href="/settings/editmeal/{{ $meal['id'] }}" style="float: left">
								...
								<form action="/settings/meals/{{ $meal['id'] }}" method="POST" style="float: left">
									{{ csrf_field() }}
									{{ method_field('DELETE') }}
